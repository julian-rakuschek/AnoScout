...
def pump_into_anoscout(bucket_name, count):
    ...
    BID = database.create_bucket(db, bucket_name, "scoring", "day")
    anomaly_funcs = list(mapping.items())

    for i in tqdm(range(count)):
        anomaly_func = random.choice(anomaly_funcs)
        anomaly_free = random.randint(1, 2) == 1
        anomaly_free = False
        name = f"{str(i).zfill(4)} {'anomaly-free' if anomaly_free else anomaly_func[0]}"
        while True:
            try:
                ts = get_random_anomaly_ts(name, [anomaly_func[1]], anomaly_free=anomaly_free)
...
if __name__ == '__main__':
    ...
    pump_into_anoscout("GutenTAG3", 100)
